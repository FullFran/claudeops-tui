...
func TestBuildPayloadEmptyPeriodData(t *testing.T) {
	...
	for _, want := range []string{"claudeops.cost.eur", "claudeops.tokens.total", "claudeops.sessions.count"} {
		if !names[want] {
...
func TestBuildPayloadNonZeroOneProject(t *testing.T) {
	...
	for i := range sm.Metrics {
		...
		case "claudeops.cost.eur":
			costMetric = m
		case "claudeops.tokens.total":
			tokenMetric = m
		case "claudeops.sessions.count":
			sessionMetric = m
		}
	}
...
func TestBuildPayloadMetricNames(t *testing.T) {
	...
	wantNames := []string{
		"claudeops.cost.eur",
		"claudeops.tokens.total",
		"claudeops.sessions.count",
	}
...
func TestBuildPayloadTokenTypeAttributeValues(t *testing.T) {
	...
	for i := range sm.Metrics {
		if sm.Metrics[i].Name == "claudeops.tokens.total" {
